...
async def create_account(account_id, account_info: str):
    """创建账户"""
    try:
        account = Account(
...
async def is_account_exist(account_id: str):
    """账户是否存在"""
    account = await query_account_one(account_id)
    if account:
        return True
    else:
        return False
...
async def update_account(order: Order):
    """订单成交后账户操作"""
    account = await query_account_one(order.account_id)
    pos_val = await query_position_value(order.account_id)

    if order.order_type == OrderType.BUY.value:
        await on_account_buy(account, order, pos_val)
    else:
        await on_account_sell(account, order, pos_val)


async def on_account_buy(account: dict, order: Order, pos_val: float):
    """买入成交后账户操作"""
    ...
    result = await db[account_cl].update_one( {"account_id": account["account_id"]}, {
        "$set": {
...
async def on_account_sell(account: dict, order: Order, pos_val: float):
    """卖出成交后账户操作"""
...
async def on_account_liquidation(db, account_id: str):
    """账户清算"""
    account = await query_account_one(account_id)
    pos_val = await query_position_value(account_id)
...
async def query_account_list():
    """查询账户列表"""
    result = await db[account_cl].find()
    return result


async def query_account_one(account_id: str):
    """查询账户信息"""
    if account_id:
...
async def on_orders_arrived(order: Order):
    """订单到达"""
    # 接收订单前的验证
    if Settings.VERIFICATION:
        result, msg = on_front_verification(order)
        if not result:
            return result, msg

    return on_orders_book_insert(order)


async def on_orders_insert(order: Order):
    """订单插入"""
...
async def on_orders_exist(token: str, order_id: str):
    """查询订单是否存在"""
...
async def on_order_update(order: Order):
    """订单状态更新"""
    result, order_old = query_order_one(order.account_id, order.order_id)
    result = await db[trade_cl].update_many({"order_id": order.order_id, "account_id": order.account_id}, {
        "$set": {
            "status": order.status,
...
async def on_order_deal(order: Order):
    """订单成交处理"""
    # 持仓处理
    await on_position_update(order)
...
async def query_order_status(account_id: str, order_id: str):
    """查询订单情况"""
...
async def query_orders(account_id: str):
    """查询交割单"""
...
async def query_order_one(account_id: str, order_id: str):
    """查询一条订单数据"""
    order = await db[trade_cl].find_one({"order_id": order_id, "account_id": account_id})
    if order:
        return True, order
...
async def on_orders_book_insert(order: Order):
    """订单薄插入订单"""
...
async def query_position(account_id: str):
    """查询所有持仓信息"""
...
async def query_position_one(account_id: str, symbol: str):
    """查询某一只证券的持仓"""
...
async def query_position_value(token: str):
    """查询账户市值"""
    pos = query_position(token)
...
async def on_position_insert(order: Order, cost: float):
    """持仓增加"""
    profit = cost * -1
...
async def on_position_update(order: Order):
    """订单成交后持仓操作"""
    if order.order_type == OrderType.BUY.value:
        await on_position_append(order)
    else:
        await on_position_reduce(order)


async def on_position_append(order: Order):
    """持仓增长"""
    result, pos_o = await query_position_one(order.account_id, order.pt_symbol)
    ...
    if result:
        ...
        return await db[position_cl].update_one({"pt_symbol": order.pt_symbol, "account_id": order.account_id}, {
            "$set": {
        ...
        return await on_position_insert(order, cost)


async def on_position_reduce(order: Order):
    """持仓减少"""
    result, pos_o = query_position_one(order.account_id, order.pt_symbol)
...
async def on_position_liquidation(account_id, price_dict: dict = None):
    """持仓清算"""
    pos_list = await query_position(account_id)
    if isinstance(pos_list, list):
        for pos in pos_list:
            ...
            # 解除账户冻结
            await on_position_frozen_cancel(account_id, pos)


async def on_position_update_price(token: str, pos: dict, price: float):
    """更新持仓价格并解除冻结"""
    volume = pos["volume"]
    if volume:
        profit = (price - pos["now_price"]) * pos["volume"] + pos["profit"]

        return await db[position_cl].update_many({"pt_symbol": pos["pt_symbol"]},{
            "$set": {
...
async def on_position_frozen_cancel(account_id: str, pos: dict):
    """持仓解除冻结"""
    volume = pos["volume"]
    if volume:
        raw_data = {}
...
async def on_front_verification(order: Order):
    """订单前置验证"""
    # 验证市场是否开启
    ...
    # 验证账户是否存在
    if not is_account_exist(order.account_id):
        return False, "账户不存在"

    if order.order_type == OrderType.BUY.value:
        return account_verification(order)
    else:
        return position_verification(order)


async def account_verification(order: Order):
    """订单账户验证"""
    money_need = order.volume * order.order_price * (1 + Settings.COST)
    account = await query_account_one(order.account_id)

    if account["available"] >= money_need:
        await on_buy_frozen(account, money_need)
        return True, ""
    else:
        return False, "账户资金不足"


async def position_verification(order: Order):
    """订单持仓验证"""
    pos_need = order.volume
    result, pos = query_position_one(order.account_id, order.pt_symbol)

    if result:
        if pos["available"] >= pos_need:
            await on_sell_frozen(pos, order.volume)
            return True, ""
        else:
            return False, "可用持仓不足"
    else:
...
async def on_buy_frozen(account, pay: float):
    """买入资金冻结"""
...
async def on_sell_frozen(pos, vol: float):
    """卖出证券冻结"""
...
async def on_order_cancel(order: Order):
    """取消订单或订单被拒单操作"""
    await on_order_update(order)

    if order.order_type == OrderType.BUY.value:
        return await on_buy_cancel(order)
    else:
        return await on_sell_cancel(order)


async def on_buy_cancel(order: Order):
    """买入订单取消"""
...
async def on_sell_cancel(order: Order):
    """卖出取消"""
    result, pos = query_position_one(order.account_id, order.pt_symbol)
...
async def on_liquidation(account_id: str, price_dict: dict = None):
    """清算"""
    # 更新所有持仓最新价格和冻结证券
    await on_position_liquidation(db, account_id, price_dict)

    # 更新账户市值和冻结资金
    await on_account_liquidation(db, account_id)
